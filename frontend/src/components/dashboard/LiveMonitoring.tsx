    ...
    return (
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 bg-white rounded-xl border border-slate-200 p-6 flex flex-col items-center justify-center h-80">
          ...
          <div className="skeleton w-40 h-4 mt-4" />
        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 bg-white rounded-xl border border-slate-200 p-6 flex flex-col items-center justify-center">
          <div className="relative">
            <AQIGauge aqi={formatAQI(airQuality.aqi)} label={aqiLabel} color={aqiColor} />
          </div>
          <div className="mt-4 text-center">
            <span className="text-xs text-slate-400">Data source: {airQuality.dataSource || 'Open-Meteo / Fallback'}</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          <h3 className="text-sm font-semibold text-slate-700 mb-2">City Health Metrics</h3>
          <div className="space-y-2 text-sm text-slate-600">
            <div className="flex justify-between py-1 border-b border-slate-50 last:border-0">
              <span className="text-slate-500">Population</span>
              <span className="font-medium text-slate-800">{city.population}</span>
            </div>
            <div className="flex justify-between py-1 border-b border-slate-50 last:border-0">
              <span className="text-slate-500">Area</span>
              <span className="font-medium text-slate-800">{city.area}</span>
            </div>
            <div className="flex justify-between py-1 border-b border-slate-50 last:border-0">
            ...
            <div className="flex justify-between py-1">
              ...
              <span className="font-medium" style={{ color: aqiColor }}>{aqiLabel}</span>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          <h3 className="text-sm font-semibold text-slate-700 mb-2">Environmental Intelligence</h3>
          <div className="space-y-2">
            {airQuality.aqi > 200 && (
              <div className="px-3 py-2 bg-red-50 text-red-700 rounded-lg text-xs font-medium flex items-center gap-2">
                <span className="w-1.5 h-1.5 rounded-full bg-red-500 shrink-0" />
                High pollution — consider issuing health advisory
              </div>
            )}
            {airQuality.windSpeed < 5 && (
              <div className="px-3 py-2 bg-amber-50 text-amber-700 rounded-lg text-xs font-medium flex items-center gap-2">
                <span className="w-1.5 h-1.5 rounded-full bg-amber-500 shrink-0" />
                Low wind — pollution may accumulate
              </div>
            )}
            {airQuality.humidity > 70 && (
              <div className="px-3 py-2 bg-blue-50 text-blue-700 rounded-lg text-xs font-medium flex items-center gap-2">
                <span className="w-1.5 h-1.5 rounded-full bg-blue-500 shrink-0" />
                High humidity — secondary particle formation likely
              </div>
            )}
            {airQuality.aqi <= 100 && (
              <div className="px-3 py-2 bg-teal-50 text-teal-700 rounded-lg text-xs font-medium flex items-center gap-2">
                <span className="w-1.5 h-1.5 rounded-full bg-teal-500 shrink-0" />
                Air quality is acceptable — continue monitoring
              </div>
            )}
            {airQuality.aqi > 100 && airQuality.aqi <= 200 && (
              <div className="px-3 py-2 bg-yellow-50 text-yellow-700 rounded-lg text-xs font-medium flex items-center gap-2">
